...
function findBreakPoints(text) {
      ...
      return [...byIndex.entries()]
        .map(([index, priority]) => ({ index, priority, weakEnd: endsWeak(wordBeforeIndex(text, index)) }))
...
function splitIntoChunks(text, n) {
      ...
      const cuts = [];
      let start = 0;
      const scoreOf = (p, target) => Math.abs(p.index - target) + p.priority * 6 + (p.weakEnd ? 10 : 0);
      for (let i = 1; i < n; i++) {
...
function reflow(rawCues, settings) {
  ...
  const merged = [];
    for (const cue of cues) {
          ...
          if (prev) {
                  ...
                  if (gap <= 300 && fitsLines && combinedDuration <= maxDurationMs && prev.italic === cue.italic) {
  ...
  cues = cues.map((cue) => {
        const lines = wrapLinesSmart(cue.text, maxCharsPerLine);
        const duration = cue.endMs - cue.startMs;
