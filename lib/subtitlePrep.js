...
// (never mid-clause), because this tool can't tell what's safe to cut from
// a sentence in general. Anything still too dense after this is left alone
// and reported as a warning, same as before.

const MID_FILLER_PHRASES = ["you know", "i mean", "basically", "honestly", "to be honest"];
const TRAILING_CLAUSES = ["if that makes sense", "if you will", "or something", "or whatever", "you know what i mean"];

function capitalizeFirst(text) {
 return text.replace(/^(\s*)([a-z])/, (m, lead, ch) => lead + ch.toUpperCase());
}

function stripFillerPhrases(text) {
 let out = text;
 for (const phrase of MID_FILLER_PHRASES) {
  const p = phrase.replace(/ /g, "\\s+");
  const re = new RegExp(`(^|,\\s*)\\b(${p})\\b\\s*(,\\s*|[.!?]|$)`, "gi");
  out = out.replace(re, (match, lead, _phrase, trail) => {
   if (lead === "") return "";
...
function stripTrailingClause(text) {
 let out = text;
 for (const phrase of TRAILING_CLAUSES) {
  const p = phrase.replace(/ /g, "\\s+");
...
function reflow(rawCues, settings) {
 ...
 const split = [];
    ...
    const splitsForLines = Math.ceil(lineCount / maxLines);
     ...
     // cps alone shouldn't force a split once the cue already fits maxLines
...
const edits = [];
 if (shouldStrip) {
  cues = cues.map((cue) => {
   ...
   let text = stripTrailingClause(cue.text);
